# 公告

[2024年初计算机设计大赛项目计划](Notification/2024年设计比赛项目计划.html)

# 计算机科学

## 运维

[Debian 系统命令](ComputerScience/Debian系统命令.html)

[Debian 建立 SSH 连接](ComputerScience/Debian建立SSH连接.html)

[Debian 环境下搭建 LAMP](ComputerScience/Debian搭建LAMP环境.html)

[Debian 运行 Code-Server](ComputerScience/Debian运行Code-Server.html)

## 工具

[Git 使用说明](ComputerScience/Git使用说明.html)

[Vim 使用说明](ComputerScience/Vim使用说明.html)

[Typora 使用说明](ComputerScience/Typora使用说明.html)

[Markdown 语法](ComputerScience/Markdown语法.html)

[Visual Studio Code 如何运行代码](ComputerScience/Visual Studio Code运行代码.html)

## 编程笔记

[Python 笔记](ComputerScience/Python.html)

[C 语言标准库](ComputerScience/C语言标准库.html)

[C 语言文件操作](ComputerScience/C语言文件操作.html)

[C++ 标准库](ComputerScience/C++标准库.html)